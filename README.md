...
- **Иллюстрация** — генерация иллюстрации на основе статьи: создание промпта через OpenRouter AI и генерация изображения через Hugging Face
...
- **OpenRouter AI** — для AI-обработки статей (перевод, генерация описаний, тезисов, постов и создания промптов для иллюстраций)
- **Hugging Face Inference API** — для генерации изображений (модель stabilityai/stable-diffusion-xl-base-1.0)
...
Для работы AI-функций требуются API ключи:
- **OpenRouter** — для перевода, описания, тезисов, постов для Telegram и создания промптов для иллюстраций
...
4. Скопируйте токен
5. **Важно**: Используйте переменную `HUGGINGFACE_API_KEY` (без подчеркивания между HUGGING и FACE)
...
HUGGINGFACE_API_KEY=ваш_api_ключ_от_hugging_face
```
...
3. Добавьте переменные:
   ...
   - **Value**: ваш API ключ от Hugging Face
